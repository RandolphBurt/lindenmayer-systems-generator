...
export class AppComponent {
  ...
  private processDefinition(): void {
      ...
      let validationResult = this.definitionVAlidator.validate(this.lindenmayerSystemDefinition);
      if (validationResult.result === true && this.iterationCount > 0) {
          // Iterate over the axiom and generated results 'iterationCount' times....
          let result = this.rulesProcessor.process(this.lindenmayerSystemDefinition, this.iterationCount);

          // Calculate the space/rectangular-size required to draw the resultant shape
          let lindenmayerSystemResultBoundaryCalculator = this.resultBoundaryCalculatorFactory.Create();
          this.resultParser.parseResult(lindenmayerSystemResultBoundaryCalculator, this.lindenmayerSystemDefinition, result);

          let diffX = lindenmayerSystemResultBoundaryCalculator.maxX - lindenmayerSystemResultBoundaryCalculator.minX;
          let diffY = lindenmayerSystemResultBoundaryCalculator.maxY - lindenmayerSystemResultBoundaryCalculator.minY;

          let scale: number = Math.min(canvas.width / diffX, canvas.height / diffY);
          let startX: number = (-1 * lindenmayerSystemResultBoundaryCalculator.minX) + (0.5 * ((canvas.width / scale) - diffX));
          let startY: number = (-1 * lindenmayerSystemResultBoundaryCalculator.minY) + (0.5 * ((canvas.height / scale) - diffY));
          ...
          let lindenmayerSystemResultRenderer = this.resultRendererFactory.Create(canvasContext, startX, startY);
  ...
  loadAndRender(): void {
      let chosenDefinition = this.libraryDefinitions.filter((x) => x.title === this.selectedPredefinedDefinition)[0];
